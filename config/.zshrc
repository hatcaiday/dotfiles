...
export ZSH="/home/do.thi.nhung/.oh-my-zsh"
...
gcp() {
  git fetch origin pull/$1/head:pullrequest/$1
...
gff() {
  ...
  branch_name=$(git symbolic-ref --short -q HEAD);
  if gd | grep "binding.pry" | grep "debugger"
  then
    echo "Binding or debugger pry detected";
    return 0;
  fi
  git push origin $branch_name -f;
}
...
#pull FE and run yarn gulp
yg() {
  git pull origin front-end-master
  cd front-end
  [[ -s "$HOME/.rvm/scripts/rvm" ]] && source "$HOME/.rvm/scripts/rvm"
  yarn gulp dev -a 3001
}
